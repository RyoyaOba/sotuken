...
class CustomDataset:
    def __init__(self, folder_path: str, seed: int = 35, single_y: bool = False) -> None:
        ...
        for label_folder in os.listdir(self.folder_path)[::-1]:
            ...
            if label is not None:
                ...
                for csv_file in os.listdir(label_folder_path)[:2622]:
    ...
    # def __call__(self, size: int, batch_size: int = 10, train_split: float = 0.8, val_split: float = 0.1,
    #              test_split: float = 0.1) -> Any:
    #     assert (train_split + test_split + val_split) == 1

    #             # Limit sample_size based on label 1 data count
    #     #sample_size = min(size, self.label_1_count)
    #     # シャッフル
    #     ds = self.dataset.shuffle(10000, seed=self.seed)
...
# ... (previous code remains the same)
        ...
        label_0_csv_count = 0
        label_1_csv_count = 0
        thumb_1_folder = '/app/Oba_卒業研究A/2023年度歩容測定実験/venus3d_data/normalization_data/sotukenB_clustering/sotukenB_thumb_1'
